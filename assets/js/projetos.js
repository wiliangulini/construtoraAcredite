...
let Galeria = () => {
  Galery.map((foto, index) => {
    
    let galeria = foto.galeria;
    ...
    for(let i in galeria) {
      ...
      col.querySelector('.link').addEventListener('click', (e) => {
        ...
        //src da imagem
        let endereco = galeria[key];

        button.classList.remove('none');
        buttonPrev.classList.remove('none');

        if(localStorage.getItem('chave') == "81") {
      ...
      col.querySelector('.photoHome').src = galeria[i];  
    }
